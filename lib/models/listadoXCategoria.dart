...
List<ListadoXCategoria> listXCatFromJson(String str) {
  ...
  return new List<ListadoXCategoria>.from(
      jsonData.map((x) => ListadoXCategoria.fromJson(x)));
}
...
class ListadoXCategoria {
  ...
  List<ListadoXCategoria> createListXCate(idListado, List<Categoria> list) {
    ...
    for (var i = 0; i < list.length; i++) {
      lxc = new ListadoXCategoria(
          idCategoria: list[i].idCategoria, idListado: idListado);
      listado.add(lxc);
    }
    return listado;
  }

  factory ListadoXCategoria.fromJson(Map<String, dynamic> json) =>
      new ListadoXCategoria(
